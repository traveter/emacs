(setq auto-mode-alist
      ...
      ;; (append '(("\\.tpl\\'" . web-mode)) auto-mode-alist))
(when (autoload-if-found 'web-mode "web-mode" nil t)
  (eval-after-load 'web-mode
    '(progn
       (setq my:css-dictionary (ac-mode-dictionary 'css-mode)
	     my:html-dictionary (ac-mode-dictionary 'html-mode)
	     my:js-dictionary (ac-mode-dictionary 'javascript-mode)
	     my:smarty-dictionary (ac-mode-dictionary 'smarty-mode)
	     )
       (defun my:local-ac (dictionary)
	 (interactive)
	 (let ((ac-sources '(ac-source-yasnippet
			     ac-source-words-in-same-mode-buffers
			     ac-source-words-in-buffer
			     ))
	       (ac-user-dictionary dictionary)
	       )
	   (ac-clear-dictionary-cache)
	   (auto-complete))
	 )
       (defun my:local-css-ac ()
	 (interactive)
	 (let ((ac-sources '(ac-source-yasnippet
	       ...
	       (ac-user-dictionary my:css-dictionary)
	       )
	   (ac-clear-dictionary-cache)
	   (auto-complete))
	 )
       (define-keys web-mode-map
	 ((kbd "C-M-k") '(lambda () (interactive) (my:local-css-ac)))
	 ((kbd "C-M-h") '(lambda () (interactive) (my:local-ac my:html-dictionary)))
	 ((kbd "C-M-j") '(lambda () (interactive) (my:local-ac my:js-dictionary)))
	 ((kbd "C-M-l") '(lambda () (interactive) (my:local-ac my:smarty-dictionary)))
	 )
  )))
